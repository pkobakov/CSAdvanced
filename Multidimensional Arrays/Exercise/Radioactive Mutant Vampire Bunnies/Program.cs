...
static void AddTheExistingBunnies(int rows, int cols, char[,] matrix, List<int> bunnies)
{
    for (int row = 0; row < rows; row++)
    {
        for (int col = 0; col < cols; col++)
...
static void SpreadBunnies(char[,] matrix, int bunnyRow, int bunnyCol)
{
    if (bunnyRow - 1 >= 0)
    {
...
static bool Iswon(char[,] matrix, int currentPlayerRow, int currentPlayerCol)
{
